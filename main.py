...
def parse_args(args):
    ...
    parser.add_argument("-t", "--biases", dest="biasfile",\
                        ...
                        ICE for each locus are read from BIASFILE",\
                        required=False)
    
    parser.add_argument("-p", "--passes", dest="noOfPasses",type=int,\
                        help="OPTIONAL: number of passes after the initial\
                        (before) fit. DEFAULT is 0 (after)", required=False)
                        #RUNBY changed to default of 0
...
def main():
    ...
    (mainDic,observedInterAllSum,observedIntraAllSum,observedIntraInRangeSum, biasDic)=read_All_Interactions(mainDic,contactCountsFile,noOfFrags) # from read_All_Interactions
