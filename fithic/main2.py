...
def main():
    ...
    (mainDic,observedInterAllSum,observedIntraAllSum,observedIntraInRangeSum, biasDic) = read_Interactions(mainDic, contactCountsFile, biasFile, distLowThres, distUpThres)
    bins = makeBinsFromInteractions(mainDic,noOfBins, observedIntraInRangeSum, distLowThres, distUpThres, distScaling)
    
    #Enumerate (fast version) or generate (otherwise) all possible pairs of fragments within the range of interest.
    (mainDic,noOfFrags, maxPossibleGenomicDist, possibleIntraInRangeCount, interChrProb, baselineIntraChrProb)= generate_FragPairs2(mainDic, fragsFile, mappThres, resolution, distLowThres, distUpThres)
    
    with open(outputPath+"MAINDIC", 'w') as f:
        for i in mainDic:
            f.write("KEY:     %s\n" % i)
            f.write("VALUE[Npairs]:    %s\n" % mainDic[i][0])
            f.write("VALUE[TotCC]:     %s\n" % mainDic[i][1])
            f.write("\n")
            f.write("\n")
    sys.exit(2)
...
def read_Interactions(mainDic, contactCountsFile, biasDic, distLowThres, distUpThres):
...
def makeBinsFromInteractions(mainDic,noOfBins, observedIntraInRangeSum, distLowThres, distUpThres, distScaling):
...
def generate_FragPairs2(mainDic, fragsfile, mappThres, resolution, distLowThres, distUpThres):
    if resolution:
        with open(logfile, 'a') as log:
    ...
    if resolution:
        ...
        for ch in allFragsDic: 
            countIntraPairs = 0
            fragsPerChr = sorted(list(allFragsDic[ch]))
            templen = len(fragsPerChr)
            possibleInterAllCount += (noOfFrags-templen)*templen
            for x in range(templen):
                d = 0
                for y in range(x+1,templen):
                    interactionDistance = abs(float(fragsPerChr[x])-float(fragsPerChr[y]))
                    maxPossibleGenomicDist = max(maxPossibleGenomicDist, interactionDistance)
                    if myUtils.in_range_check(interactionDistance, distLowThres,distUpThres):
                        mainDic[interactionDistance][0] += templen-d
                        d+=1 #FIGURING OUT THIS
                        countIntraPairs += 1
                    possibleIntraAllCount += 1
            possibleIntraInRangeCount += countIntraPairs
            sys.stderr.write("Chromosome " +repr(ch) +",\t"+str(templen) +" mappable fragments, \t"+str(countIntraPairs)\
