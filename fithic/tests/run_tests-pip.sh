...
for i in Dixon_hESC_HindIII_hg18_combineFrags10_chr1 Dixon_mESC_HindIII_mm9_combineFrags10_chr1; do
   fithic -l "$i" -f $inF/$i.gz -i $inI/$i.gz -o outputs/$i -L $distLowThres -U $distUpThres -b $noOfBins -p $noOfPasses --quiet -x intraOnly
done

# without normalization - fixed size windows
noOfBins=50
for i in Dixon_hESC_HindIII_hg18_w40000_chr1; do
   fithic -l "$i" -f $inF/$i.gz -i $inI/$i.gz -L $distLowThres -U $distUpThres -b $noOfBins -p $noOfPasses -o outputs/$i --quiet -x All
done
...
for i in hIMR90_100kb_hg19; do
    fithic -l "$i" -i $inI/${i}_cc.gz -f $inF/${i}_frags.gz -b $noOfBins -p $noOfPasses -o outputs/${i}.interOnly -x interOnly
    fithic -l "$i" -i $inI/${i}_cc.gz -f $inF/${i}_frags.gz -b $noOfBins -p $noOfPasses -o outputs/${i}.all -x All
done


#USE BELOW IF WANTING TO TRY NONHUMAN/MOUSE ORGANISMS
#for i in RINGS; do
#    fithic -l "$i" -i $inI/${i}.gz -f $inF/${i}.gz -b $noOfBins -p $noOfPasses -o outputs/${i} -x All
#done
